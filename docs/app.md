...
- **State** lives in `@lit-labs/signals` stores under `@common/` — see [ADR-0004](adr/0004-signals-for-state.md). The stores are `data`, `edits`, `selection`, `view-state`, `interaction-mode` and `panels`, plus the `urlSignal()` primitive in `url-state`.
...
The desktop entry lives at `src/server/index.ts` (the name is required because Electrobun's launcher hardcodes `app/bun/index.js`); the dev entry lives at `src/server/dev.ts`; the E2E entry lives at `tests/server.ts`. All three open a library session and differ only in what they supply it and what they wrap it in: which static root serves the client (the bundled views vs `src/client`), a per-response hook (FDA detection vs request logging vs none), how a failed library resolution is surfaced (a recoverable dialog with Retry vs a message and a non-zero exit vs no resolution at all), and where the support dir and cache root live. The E2E entry additionally substitutes the adapters that would otherwise reach a real library — the photos library, the writer, the item source, the album roster, and the active-library guard — so a run touches neither Photos.sqlite nor AppleScript.

Where the cache root lives stays with the entries rather than moving into the session: the `~/Library/Caches` choice exists for Time Machine's path policy and the packaged app's identity, which is the desktop entry's concern and not a fact about libraries. What the session owns is everything _under_ that root.
